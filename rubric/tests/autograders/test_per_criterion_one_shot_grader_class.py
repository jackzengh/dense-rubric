    ...
    async def generate_with_issue(system_prompt: str, user_prompt: str) -> OneShotOutput:
        return OneShotOutput(
            criteria_evaluations=[
                CriterionEvaluation(criterion_idx=0, criterion_status="MET", explanation="Test"),
                CriterionEvaluation(criterion_idx=1, criterion_status="MET", explanation="Test"),
                CriterionEvaluation(criterion_idx=2, criterion_status="MET", explanation="Test"),
                CriterionEvaluation(
                    criterion_idx=3,
...
async def test_all_negative_criteria_all_met_returns_zero_score():
    ...
    async def generate_all_errors(system_prompt: str, user_prompt: str) -> OneShotOutput:
        return OneShotOutput(
            criteria_evaluations=[
                CriterionEvaluation(
                    criterion_idx=0, criterion_status="MET", explanation="Has errors"
                ),
                CriterionEvaluation(
                    criterion_idx=1,
